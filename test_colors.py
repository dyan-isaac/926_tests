...
################
def test_colormap_with_alpha():
    """Test the with_alpha method of Colormap."""
    # Create a simple colormap for testing
    cmap = mcolors.ListedColormap(['red', 'blue'], name='test_alpha')

    # Test with valid alpha
    alpha_value = 0.5
    alpha_cmap = cmap.with_alpha(alpha_value)
    assert alpha_cmap is not cmap  # Should return a copy

    # Apply the alpha and check directly using an array of values
    test_value = 0.5  # Middle of the range
    rgba_with_alpha = alpha_cmap(test_value)
    rgba_original = cmap(test_value)

    # The RGB values should be the same, only alpha should differ
    assert np.allclose(rgba_with_alpha[:3], rgba_original[:3])
    # Alpha should be the specified value
    assert np.isclose(rgba_with_alpha[3], alpha_value)
    # Original should have alpha=1.0
    assert np.isclose(rgba_original[3], 1.0)

    # Test error cases
    with pytest.raises(TypeError, match="'alpha' must be numeric"):
        cmap.with_alpha("invalid")

def test_colormap_is_gray():
    """Test the is_gray method of Colormap."""
    # Test with grayscale colormap
    gray_cmap = mcolors.ListedColormap(['black', 'gray', 'white'], name='test_gray')
    assert gray_cmap.is_gray()

    # Test with non-grayscale colormap
    color_cmap = mcolors.ListedColormap(['red', 'blue'], name='test_color')
    assert not color_cmap.is_gray()


def test_colormap_resampled():
    """Test the resampled method of Colormap."""
    # Create a colormap with n entries and resample to m entries
    original_size = 10
    new_size = 20
    cmap = mcolors.ListedColormap(['red', 'blue', 'green'], name='test_resample')

    # Test resampling
    resampled_cmap = cmap.resampled(new_size)
    assert resampled_cmap.N == new_size

    # Initialize the colormap to create the lookup table
    resampled_cmap._init()

    # Check that the lookup table has the right size
    # (N + 3 special colors: under, over, bad, masked)
    expected_lut_size = new_size + 3
    assert resampled_cmap._lut.shape[0] == expected_lut_size, f"Expected LUT size {expected_lut_size}, got {resampled_cmap._lut.shape[0]}"

    # Create a mock colormap class that only implements _resample
    class OldColormapClass(mcolors.Colormap):
        def __init__(self):
            super().__init__("old_style", N=10)
            self._isinit = True

        def _init(self):
            # Create lookup table with correct size
            self._lut = np.zeros((self.N + 4, 4))
            self._lut[:, 3] = 1.0  # Set alpha to 1

        def _resample(self, lutsize):
            new_cmap = OldColormapClass()
            new_cmap.N = lutsize
            return new_cmap

    # Test warning with old-style colormap
    old_cmap = OldColormapClass()
    with pytest.warns(UserWarning, match="now public API"):
        resampled_old = old_cmap.resampled(20)
    assert resampled_old.N == 20


def test_colormap_reversed_abstract():
    """Test the reversed method raises NotImplementedError in base class."""

    # Create an instance of a minimal Colormap subclass
    class MinimalColormap(mcolors.Colormap):
        def _init(self):
            self._lut = np.zeros((self.N + 3, 4))
            self._lut[:, 3] = 1.0
            self._isinit = True

    cmap = MinimalColormap("minimal")
    with pytest.raises(NotImplementedError):
        cmap.reversed()

    # Test with actual colormap implementations
    viridis = matplotlib.colormaps["viridis"]
    viridis_r = viridis.reversed()

    assert viridis_r.name == "viridis_r"
    # Initialize both colormaps to create lookup tables
    viridis._init()
    viridis_r._init()
    # Check that colors are reversed
    assert np.array_equal(viridis_r._lut[:-3], viridis._lut[:-3][::-1])