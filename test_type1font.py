...
def test_combined_transformations():
    """Test applying multiple transformations at once."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    # Apply both slant and extend transformations
    transformed = font.transform({'slant': 0.5, 'extend': 0.75})

    # Verify font name reflects both transformations
    # Instead of checking for exact text, look for the font name with flexible matching
    font_name_pattern = b'/FontName/CMR10_'
    assert any(font_name_pattern in line and b'Extend_750' in line and b'Slant_500' in line
               for line in transformed.parts[0].splitlines()), "Font name doesn't reflect transformations"


def test_font_prop_access():
    """Test accessing various font properties."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    # Test font property access
    assert 'FontName' in font.prop
    assert font.prop['FontName'] == 'CMR10'
    assert 'FontType' in font.prop


def test_binary_section_handling():
    """Test handling of binary sections in the font."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    # Check that binary sections are properly handled
    assert len(font.parts) == 3

    # The second part should be the encrypted binary section
    encrypted = font.parts[1]
    decrypted = t1f.Type1Font._decrypt(encrypted, 'eexec')

    # Verify we can decrypt it
    assert b'Subrs' in decrypted
    assert b'CharStrings' in decrypted


def test_custom_encryption():
    """Test custom encryption/decryption with different keys."""
    # Test with custom data and keys
    test_data = b'Testing custom encryption with Type1Font'

    # Test with 'eexec' key
    encrypted_eexec = t1f.Type1Font._encrypt(test_data, 'eexec')
    decrypted_eexec = t1f.Type1Font._decrypt(encrypted_eexec, 'eexec')
    assert decrypted_eexec == test_data

    # Test with 'charstring' key
    encrypted_char = t1f.Type1Font._encrypt(test_data, 'charstring')
    decrypted_char = t1f.Type1Font._decrypt(encrypted_char, 'charstring')
    assert decrypted_char == test_data

    # The two encryption methods should produce different results
    assert encrypted_eexec != encrypted_char
...
def test_binary_section_character_data():
    """Test character data extraction from binary sections."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    # Verify specific character entries exist in the decrypted data
    decrypted = t1f.Type1Font._decrypt(font.parts[1], 'eexec')

    # Look for common characters
    assert b'/space' in decrypted
    assert b'/period' in decrypted
    assert b'/A' in decrypted

    # Check character string dictionary structure
    assert b'begin' in decrypted
    assert b'end' in decrypted


def test_binary_section_roundtrip():
    """Test encryption and decryption roundtrip for binary sections."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    # Take a sample of the decrypted data
    decrypted = t1f.Type1Font._decrypt(font.parts[1], 'eexec')
    sample = decrypted[:100]  # Take first 100 bytes

    # Re-encrypt it
    re_encrypted = t1f.Type1Font._encrypt(sample, 'eexec')

    # Decrypt again and verify it matches the original
    re_decrypted = t1f.Type1Font._decrypt(re_encrypted, 'eexec')
    assert re_decrypted == sample


def test_font_prop_access_basic():
    """Test accessing basic font properties."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)
...
def test_font_prop_weight():
    """Test accessing font weight property."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    assert 'Weight' in font.prop
    assert font.prop['Weight'] == 'Medium'


def test_font_prop_fixed_pitch():
    """Test accessing isFixedPitch property."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    assert 'isFixedPitch' in font.prop
    assert font.prop['isFixedPitch'] is False


def test_font_prop_italic_angle():
    """Test accessing ItalicAngle property."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    assert 'ItalicAngle' in font.prop
    assert font.prop['ItalicAngle'] == 0


def test_font_prop_encoding():
    """Test accessing Encoding properties."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    assert 'Encoding' in font.prop
    assert isinstance(font.prop['Encoding'], dict)
    assert 5 in font.prop['Encoding']
    assert font.prop['Encoding'][5] == 'Pi'


def test_font_prop_char_strings():
    """Test accessing CharStrings properties."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    assert 'CharStrings' in font.prop
    assert isinstance(font.prop['CharStrings'], dict)
    assert 'Pi' in font.prop['CharStrings']
    assert isinstance(font.prop['CharStrings']['Pi'], bytes)


def test_font_prop_paint_type():
    """Test accessing PaintType property."""
    filename = os.path.join(os.path.dirname(__file__), 'cmr10.pfb')
    font = t1f.Type1Font(filename)

    assert 'PaintType' in font.prop
    # PaintType is typically 0 (filled) or 1 (stroked)
    assert font.prop['PaintType'] in (0, 1)
