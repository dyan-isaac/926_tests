...
class TestContourLabelerEventHandler(unittest.TestCase):
    def setUp(self):
        ...
        X, Y = np.meshgrid(x, y)
        Z = X ** 2 + Y ** 2
        self.cs = self.ax.contour(X, Y, Z)

        # Mock methods we'll check
        self.cs.pop_label = Mock()
        self.cs.add_label_near = Mock()
        self.cs.axes.get_figure().canvas.draw = Mock()
        self.cs.axes.get_figure().canvas.stop_event_loop = Mock()
        self.cs.axes.contains = Mock(return_value=(True, None))

    def test_middle_mouse_button_stops_event_loop(self):
        event = Mock(name="button_press_event",
                     button=MouseButton.MIDDLE,
                     inaxes=self.ax)  # Add inaxes attribute

        # Mock the specific checks the handler might be performing
        event.name = "button_press_event"  # Ensure event name is set explicitly
        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.axes.get_figure().canvas.stop_event_loop.assert_called_once()
        self.cs.pop_label.assert_not_called()
        self.cs.add_label_near.assert_not_called()

    def test_escape_key_stops_event_loop(self):
        event = Mock(name="key_press_event", key="escape")
    ...
    def test_enter_key_stops_event_loop(self):
        event = Mock(name="key_press_event", key="enter")
        event.name = "key_press_event"
        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.axes.get_figure().canvas.stop_event_loop.assert_called_once()
        self.cs.pop_label.assert_not_called()
    ...
    def test_right_mouse_button_pops_label(self):
        event = Mock(name="button_press_event",
        ...
        # Check assertions
        self.cs.pop_label.assert_called_once()
        self.cs.axes.get_figure().canvas.draw.assert_called_once()
        self.cs.add_label_near.assert_not_called()

    def test_backspace_key_pops_label(self):
        event = Mock(name="key_press_event", key="backspace")
        event.name = "key_press_event"
        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.pop_label.assert_called_once()
        self.cs.axes.get_figure().canvas.draw.assert_called_once()
        self.cs.add_label_near.assert_not_called()
        self.cs.axes.get_figure().canvas.stop_event_loop.assert_not_called()

    def test_delete_key_pops_label(self):
        event = Mock(name="key_press_event", key="delete")
        event.name = "key_press_event"
        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.pop_label.assert_called_once()
        self.cs.axes.get_figure().canvas.draw.assert_called_once()
    ...
    def test_inline_mode_ignores_pop_label(self):
        event = Mock(name="button_press_event",
                     button=MouseButton.RIGHT)
        contour._contour_labeler_event_handler(self.cs, True, 5, event)
        self.cs.pop_label.assert_not_called()
        self.cs.axes.get_figure().canvas.draw.assert_not_called()
        self.cs.add_label_near.assert_not_called()
        self.cs.axes.get_figure().canvas.stop_event_loop.assert_not_called()

    def test_left_mouse_button_adds_label(self):
        event = Mock(name="button_press_event",
                     button=MouseButton.LEFT,
                     x=1.0, y=1.0,
                     inaxes=self.ax)  # Add inaxes attribute

        # Ensure event name is set explicitly
        event.name = "button_press_event"

        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.add_label_near.assert_called_once_with(
            1.0, 1.0, transform=False, inline=False, inline_spacing=5)
        self.cs.axes.get_figure().canvas.draw.assert_called_once()
        self.cs.pop_label.assert_not_called()
        self.cs.axes.get_figure().canvas.stop_event_loop.assert_not_called()

    def test_key_press_adds_label(self):
        event = Mock(name="key_press_event",
                     key="a",
                     x=1.0, y=1.0,
                     inaxes=self.ax)
        event.name = "key_press_event"

        # Patch the contains method to ensure it returns True
        with patch.object(self.cs.axes, 'contains', return_value=(True, None)):
            contour._contour_labeler_event_handler(self.cs, True, 10, event)

        self.cs.add_label_near.assert_called_once_with(
            1.0, 1.0, transform=False, inline=True, inline_spacing=10)
        self.cs.axes.get_figure().canvas.draw.assert_called_once()
        self.cs.pop_label.assert_not_called()
        self.cs.axes.get_figure().canvas.stop_event_loop.assert_not_called()

    def test_none_key_press_does_nothing(self):
        event = Mock(name="key_press_event", key=None,
                     x=1.0, y=1.0)
        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.add_label_near.assert_not_called()
        self.cs.pop_label.assert_not_called()
    ...
    def test_click_outside_axes_does_nothing(self):
        event = Mock(name="button_press_event",
                     button=MouseButton.LEFT,
                     x=1.0, y=1.0)
        self.cs.axes.contains.return_value = (False, None)
        contour._contour_labeler_event_handler(self.cs, False, 5, event)
        self.cs.add_label_near.assert_not_called()
        self.cs.pop_label.assert_not_called()
        self.cs.axes.get_figure().canvas.draw.assert_not_called()
